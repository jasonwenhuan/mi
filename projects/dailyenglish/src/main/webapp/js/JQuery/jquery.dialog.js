...
var JqueryDialog = {
	...
	Open:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight,field,isCreate){
		JqueryDialog.init(dialogTitle, iframeSrc, iframeWidth, iframeHeight, true, true, true,field,isCreate);
	},
	...
	Open1:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight, isSubmitButton, isCancelButton, isDrag,field,isCreate){
		JqueryDialog.init(dialogTitle, iframeSrc, iframeWidth, iframeHeight, isSubmitButton, isCancelButton, isDrag,field,isCreate);
	},
	...
	/// <param name="isCancelButton">是否呈现“取消”按钮</param>
	/// <param name="isDrag">是否支持拖拽</param>
	init:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight, isSubmitButton, isCancelButton, isDrag,field,isCreate){
		JqueryDialog.isCreate = isCreate;
		//获取客户端页面宽高
		...
		//如果用frame,技术上可调frame的css//有兴趣参考
		//不过非常复杂
		$("#jd_dialog_m_b_2").append(JqueryDialog.GetBodyStr(iframeSrc,'UTF-8'));
		//bottom
		...
		if(!JqueryDialog.isCreate){
			JqueryDialog.LoadDialogData(field);
		}
	},
	
	LoadDialogData:function(field){
		initialDialog(field);
	},
	GetBodyStr:function(html,encoding){
		DWREngine.setAsync(false); 
		var htmlStr = "";
		jUtil.convertHtmlToString(html,encoding,function(data){
			if(data != null){
				htmlStr = data;
			}
		});
	...
	/// <remark></remark>
	Ok:function(){
		submitPostInformation(JqueryDialog.isCreate);
	},
