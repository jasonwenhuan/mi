package com.chinaunicom.filterman.ws.des;
...
public class CryptUtil {
    ...
    private static SecretKey KeyGenerator(String keyStr)
    {
        ...
        try
        {
            input = md5Hex(keyStr).substring(0, 24).getBytes("GBK");
        }
        catch (UnsupportedEncodingException e)
        {
            Logging.logError("Doing CryptUtil.KeyGenerator error occurred.", e);
        }
    ...
    public static String decryptBy3DesAndBase64(String content, String keyStr, String encoding)
    {
        ...
        {
            Logging.logError("Doing CryptUtil.decryptBy3DesAndBase64 error occurred.", e);
        }
        return null;
    }
    ...
    public static byte[] cryptBy3Des(String keyStr, int cryptModel, byte iv[], byte content[])
    {
        ...
        {
            Logging.logError("Doing CryptUtil.cryptBy3Des error occurred.", e0);
        }
        catch (NoSuchPaddingException e1)
        {
            Logging.logError("Doing CryptUtil.cryptBy3Des error occurred.", e1);
        }
        catch (InvalidKeyException e2)
        {
            Logging.logError("Doing CryptUtil.cryptBy3Des error occurred.", e2);
        }
        catch (InvalidAlgorithmParameterException e3)
        {
            Logging.logError("Doing CryptUtil.cryptBy3Des error occurred.", e3);
        }
        catch (IllegalBlockSizeException e4)
        {
            Logging.logError("Doing CryptUtil.cryptBy3Des error occurred.", e4);
        }
        catch (BadPaddingException e5)
        {
            Logging.logError("Doing CryptUtil.cryptBy3Des error occurred.", e5);
        }
        return null;
    }
    ...
    public static String md5Hex(String content)
    {
        ...
        {
            Logging.logError("Doing CryptUtil.md5Hex error occurred.", e1);
        }
        try
        {
        ...
        {
            Logging.logError("Doing CryptUtil.md5Hex error occurred.", e);
        }
